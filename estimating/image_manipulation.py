...
### This file is obtained from https://github.com/rgeirhos/generalisation-humans-DNNs/blob/master/code/image_manipulation.py
### there are minor changes to adapted for our scripts
...
def imload_rgb(path):
    ...
    return cv2.imread(path, 0) / 255.0
...
def save_img(image, imgname, use_JPEG=False):
    ...
    if use_JPEG:
        cv2.imwrite(imgname+".JPEG", image) 
    else:
        cv2.imwrite(imgname+".png", image) 
